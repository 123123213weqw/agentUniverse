...
class WenXinLangChainInstance(QianfanChatEndpoint):
    """LangChain Qianfan chat endpoint bound to an agentUniverse LLM.

    Forwards generation, streaming and async calls to the wrapped
    agentUniverse ``llm`` instance using the WenXin/Qianfan chat protocol.
    """
    llm: LLM = None
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        """Generate chat completions asynchronously through the wrapped LLM.

        When streaming is enabled the tokens are accumulated from the async
        stream; otherwise a single chat completion call is made.

        Args:
            messages (List[BaseMessage]): The messages to pass into the model.
            stop (Optional[List[str]]): Optional list of stop words to use when generating.
            run_manager (Optional[AsyncCallbackManagerForLLMRun]): Callback manager for the run.
        Returns:
            ChatResult: The chat result generated by the model.
        """
        if self.streaming:
            completion = ""
            token_usage = {}
    ...
    async def _astream(
        ...
        """Stream chat completion chunks asynchronously from the wrapped LLM.
        ...
        Args:
            ...
            run_manager (Optional[AsyncCallbackManagerForLLMRun]): Callback manager for the run.
        Yields:
            AsyncIterator[ChatGenerationChunk]: streamed generation chunks.
        """
        params = self._convert_prompt_msg_params(messages, **kwargs)
        params["stop"] = stop
        params["stream"] = True
